...
<div class="container">
  ...
  <div class="row">
            ...
            <div class="py-0">
              <ul class="list-group">
                <button type="button"  href="#" class="page-link fs-5" @click="typeSelected = '全部';this.$route.query.keyWord = '';isLoading = true;getProducts(1,yOffset,'')">
                  <li class="list-group-item py-2 px-2 bg-"
                  :class="typeSelected == '全部'?'active':'bg-light'"
                  >
                    瀏覽全部
                  </li>
                </button>
                <button type="button"  href="#" class="page-link fs-5" v-for="type in Object.entries(typeNameList)" :key="type[0]">
                  ...
                  :class="typeSelected == type[1]?'active':'bg-light'"
                  @click="this.typeSelected = type[1];this.$route.query.keyWord = '';isLoading = true;getProducts(1,yOffset,type[1])">
                    {{ type[1] }}
                  </li>
                </button>
              </ul>
    ...
    <div class="col-12 col-sm-8 col-md-9">
      <div class="row">
        <h4 class="opacity-0 mt-6"  :class="{'opacity-100':!filterdProducts.products?.length && !isProductsLoading}" v-if="!filterdProducts.products?.length && !isProductsLoading">無相符的查詢結果</h4>
        <h4 class="opacity-0 mt-6"  :class="{'opacity-100':$route.query.keyWord?.length}">「{{ $route.query.keyWord }}」的招式機配對結果</h4>
        <div class="col-6 col-md-4 col-lg-3" v-for="product in filterdProducts.products" :key="product.id">
          <div class="card border-0 mb-4 position-relative position-relative">
            <RouterLink :to="{ path: 'product', query: { id: product.id }}" class="vl-parent">
